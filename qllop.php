        ...
        <div class="flex mt-4">
            ...
            <div class="w-3/4 bg-green-50 p-4 shadow-md">
                <div class="text-center mb-4">
                    <h2 class="text-xl font-bold text-green-700">DANH SÁCH CÁC LỚP</h2>
                </div>
                <table class="min-w-full bg-white">
                    <thead>
                        <tr>
                            ...
                            <th class="py-2 px-4 border-b">Sĩ Số</th>
                    ...
                    // Phân trang
                    $limit = 5; // số lớp mỗi trang
                    $page = isset($_GET['page']) ? (int)$_GET['page'] : 1;
                    $start_from = ($page - 1) * $limit;

                    // Truy vấn dữ liệu lớp học với giới hạn
                    $sql = "SELECT * FROM tblop LIMIT $start_from, $limit";
                    ...
                    if ($result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            echo "<td class='py-2 px-4 border-b'>" . $row["Tenlop"] . "</td>";
                            echo "<td class='py-2 px-4 border-b'>" . $row["sosv"] . "</td>";
                            echo "<td class='py-2 px-4 border-b space-x-2'>
                                    <a href='sualop.php?malop=" . $row["Malop"] . "' class='bg-blue-500 text-white px-2 py-1 rounded'><i class='fas fa-edit'></i> Sửa</a>
                                    <a href='xoalop.php?malop=" . $row["Malop"] . "' class='bg-red-500 text-white px-2 py-1 rounded' onclick=\"return confirm('Xóa lớp học này?');\"><i class='fas fa-trash'></i> Xóa</a>
                                    <a href='danhsach.php?malop=" . $row["Malop"] . "' class='bg-green-500 text-white px-2 py-1 rounded'><i class='fas fa-list'></i> Xem danh sách</a>
                                  </td>";
                            echo "</tr>";
                        }
                    } else {
                        echo "<tr><td colspan='4' class='text-center py-4'>Không có lớp học nào.</td></tr>";
                    }

                    // Tính tổng số lớp học để phân trang
                    $sql_total = "SELECT COUNT(*) AS total FROM tblop";
                ...
                <div class="flex justify-between items-center mt-4">
                    <div class="flex space-x-2">
                        ...
                        for ($i = 1; $i <= $total_pages; $i++) {
                            ...
                            echo "<a href='qllop.php?page=$i' class='px-3 py-1 rounded $active'>$i</a>";
                        }
                        ?>
                    </div>
                    <a href="themlop.php" class="text-blue-500">+ Thêm lớp</a>
                </div>
